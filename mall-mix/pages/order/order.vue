		...
		<swiper :current="tabCurrentIndex" class="swiper-box" duration="300" @change="changeTab">
			<swiper-item class="tab-content" v-for="(tabItem, tabIndex) in navList" :key="tabIndex">
				<scroll-view class="list-scroll-content" scroll-y @scrolltolower="loadData">
					...
					<!-- 订单列表 -->
					<view v-for="(item, index) in tabItem.orderList" :key="index" class="order-item" @click="navToDetail(item)">
						<view class="i-top b-b">
							<text class="time">{{ item.group_name }}</text>
							<text class="state" :style="{ color: item.stateTipColor }">{{ item.type }}</text>
						...
						<view class="action-box b-t" v-if="item.type == 1">
